...
def get_summoner_mastery_stats(summoner_id: str, region: str):
    ...
    mastery_stats = watcher.champion_mastery.by_summoner(region=region, 
...
def summoner_stats_automatized(summoner_name, region):
    ...
    summoner_mastery_stats = get_summoner_mastery_stats(summoner_id=summoner_info['id'], 
                                                        region= region)
    ...
    summoner_ranked_stats = get_summoner_ranked_stats(summoner_id=summoner_info['id'], 
                                                      region=region)

    return {'sum_info': summoner_info, 'best_champs': top_5_champs, 'mastery_points': 
            top_5_mastery_points, 'sum_ranked_stats': summoner_ranked_stats}
