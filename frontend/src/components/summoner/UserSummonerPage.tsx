  ...
  const getUser = async () => {
    try {
      const res = await axios.get(
        `${LOCALHOST_URL}${BACKEND_PORT}/summoners/${region}/${summonerName}`,
      );
      ...
      SetChampAndMastery(() =>
        helpBestChamp.reduce((acc: any, curr, index) => {
    ...
    <>
      <Parallax strength={200} className="SummonerData, container">
        ...
        <div className="statsContainer">
          <div>
            <h2>
              <b className="important">{summonerName}</b>
            </h2>
            <div className="Test">
              <img
                src={profileIcon}
          ...
          <div className="eloContainer">
            <div>
              <h4>
                Flex-Rangliste: {flexElo} {flexWinrate}{" "}
                <img src={flexIcon} width={100} height={100} />
              </h4>
            </div>
            <div>
              <h4>
                Solo-Rangliste: {elo} {winrate}{" "}
                <img src={rankedIcon} width={100} height={100} />
              </h4>
            </div>
            <PajtimGGPic />
          </div>
      ...
      <div>
        {Object.entries(champAndMastery).map(([champion, masteryPoints]) => (
          <Parallax
            className="image"
            bgImage={`splash/${champion}_0.jpg`}
            strength={500}
          >
